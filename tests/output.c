...
void output_logic(struct fqlhandle* fql, const char* resultdir)
{
	...
	sprintf(query,
		...
		BEGIN                                                                 \
	            ...
	            select 1 into [%s/logic18] where 'A' > '{'                        \
	            ...
	            select 1 into [%s/logic40] where 'hello world!' <= 'hello world!' \
	            select 1 into [%s/logic41] where 'hello world' <= 'hello world!'  \
	            select 1 into [%s/logic42] where 'hello_world!' <= 'hello world!' \
