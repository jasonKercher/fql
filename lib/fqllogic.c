...
#define try_logic_deref_(condition_)     \
	({                               \
		int ret_ = (condition_); \
		switch (ret_) {          \
		case FQL_FAIL:           \
...
#define get_integers(n0_, n1_)                                                 \
	{                                                                      \
		try_logic_deref_(expression_get_int(&n0_, self->expr[0], rg)); \
		try_logic_deref_(expression_get_int(&n1_, self->expr[1], rg)); \
	}

#define get_floats(n0_, n1_)                                                     \
	{                                                                        \
		try_logic_deref_(expression_get_float(&n0_, self->expr[0], rg)); \
		try_logic_deref_(expression_get_float(&n1_, self->expr[1], rg)); \
	}

#define get_stringviews(s0_, s1_)                                                     \
	{                                                                             \
		try_logic_deref_(expression_get_stringview(&s0_, self->expr[0], rg)); \
		try_logic_deref_(expression_get_stringview(&s1_, self->expr[1], rg)); \
	}
...
{
	long n0 = 0;
	try_logic_deref_(expression_get_int(&n0, self->expr[0], rg));

	vec* list_vec = self->in_data->expressions;
	expression** it = vec_begin(list_vec);
	for (; it != vec_end(list_vec); ++it) {
		long n1 = 0;
		try_logic_deref_(expression_get_int(&n1, *it, rg));
		if (n0 == n1) {
			return true;
		}
	}
...
{
	double n0 = 0;
	try_logic_deref_(expression_get_float(&n0, self->expr[0], rg));

	vec* list_vec = self->in_data->expressions;
	expression** it = vec_begin(list_vec);
	for (; it != vec_end(list_vec); ++it) {
		double n1 = 0;
		try_logic_deref_(expression_get_float(&n1, *it, rg));
		if (n1 == n0) {
			return true;
		}
	}
...
{
	stringview sv0;
	try_logic_deref_(expression_get_stringview(&sv0, self->expr[0], rg));

	vec* list_vec = self->in_data->expressions;
	expression** it = vec_begin(list_vec);
	for (; it != vec_end(list_vec); ++it) {
		stringview sv1;
		try_logic_deref_(expression_get_stringview(&sv1, *it, rg));
...
int fql_logic_like(logic* self, node* rg)
{
	...
	if (self->expr[1]->expr != EXPR_CONST) {
		stringview sv1;
		try_logic_deref_(expression_get_stringview(&sv1, self->expr[1], rg));
